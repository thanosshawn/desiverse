...
export default function LoginPage() {
  ...
  const handleSignIn = async (providerAction: () => Promise<void>) => {
    ...
    try {
      ...
      // toast({ title: "Logging you in...", description: "Get ready for some desi fun!" });
    } catch (error: any) {
  ...
  if (authLoading || (!authLoading && user)) { // Show loader if auth is loading or if user exists (will redirect)
    return (
